...
**Another common framing: 1D vs 2D.** Flexbox lays items out along a single axis (row *or* column) — even when it wraps onto multiple lines, each line is sized independently based on its own content, with no relationship to the lines above/below it (no shared "columns"). Grid works along both axes at once — rows and columns are defined together, so items can be placed/sized against either. This framing is common but contested: it's easy to end up building a grid-like layout in Flexbox (Bootstrap 4's grid system does exactly this), and plenty of "1D" layouts are done in Grid too (a single `grid-template-columns` row, e.g.) — so don't treat "1D vs 2D" as a hard rule for which to reach for, more a description of what naturally falls out of each one's design.

**Not mutually exclusive.** They nest freely — a Grid page skeleton with a Flexbox nav bar inside one of its cells is a completely normal, common pattern (also true in reverse: a flex item that's itself a grid container). Pick per-component, not once for the whole page.

**Browser support**: Flexbox has slightly broader/older support than Grid; Grid isn't supported in IE11 or older Edge. Rarely relevant for anything built today, but worth knowing if a layout ever needs to support very old browsers — Flexbox is sometimes suggested as a fallback for Grid in that case, though many devs find that more hassle than it's worth for what's now a vanishingly small userbase.
...
**Password pattern example**: at least 8 characters with a letter and a number:
```html
<input type="password" pattern="(?=.*[A-Za-z])(?=.*\d).{8,}" title="At least 8 characters, including a letter and a number" minlength="8" required>
```
`(?=.*[A-Za-z])` and `(?=.*\d)` are lookaheads requiring a letter/digit anywhere in the string; `.{8,}` enforces the minimum length. `title` is essential here since the default pattern-mismatch message explains nothing. Worth knowing: current security guidance (NIST and others) has shifted toward favoring **length over character-composition rules** — strict "must include 1 uppercase/1 symbol" requirements push people toward predictable substitutions and interfere with password managers generating truly random passwords, so a simple longer minimum length (`pattern=".{12,}"`) is increasingly considered the better practice than a complex composition regex.
